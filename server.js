...
let navidromeMediaProxy = null;

function setNavidromeMediaProxy(proxy) {
  if (!proxy || typeof proxy.fetch !== 'function') throw new TypeError('NAVIDROME_MEDIA_PROXY_INVALID');
  navidromeMediaProxy = proxy;
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (pn === '/api/navidrome-media') {
    if (!navidromeMediaProxy) {
      res.writeHead(503, { 'Cache-Control': 'no-store' });
      res.end();
      return;
    }
    try {
      const capability = String(url.searchParams.get('cap') || '');
      const kind = String(url.searchParams.get('kind') || '');
      const upstream = await navidromeMediaProxy.fetch(capability, kind, req.headers.range || '');
      const headers = {
        'Cache-Control': kind === 'cover' ? 'private, max-age=3600' : 'no-store',
        'Access-Control-Allow-Origin': '*',
        'Cross-Origin-Resource-Policy': 'cross-origin',
      };
      ['content-type', 'content-length', 'content-range', 'accept-ranges'].forEach((name) => {
        const value = upstream.headers && upstream.headers.get ? upstream.headers.get(name) : '';
      ...
      const invalid = error && error.code === 'NAVIDROME_MEDIA_CAPABILITY_INVALID';
      if (!res.headersSent) res.writeHead(invalid ? 403 : 502, { 'Cache-Control': 'no-store' });
...
server.setNavidromeMediaProxy = setNavidromeMediaProxy;
