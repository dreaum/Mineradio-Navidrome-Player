...
const { AudioFocusMonitor } = require('./audio-focus-monitor');
const {
...
const { NavidromeConfigStore } = require('./navidrome-config-store');
const { NavidromeService } = require('./navidrome-service');
const { NavidromeError } = require('./navidrome-client');
const { reencryptLegacyNavidromeSecrets } = require('./navidrome-secret-migration');
const {
...
registerMineradioAppScheme(protocol);

let mainWindow = null;
let localServer = null;
let mainServerPort = 0;
let navidromeService = null;
...
const audioFocusMonitor = new AudioFocusMonitor({
  ownPidProvider: () => collectAppTrimPids(),
  onConflict: (payload) => {
    if (mainWindow && !mainWindow.isDestroyed()) mainWindow.webContents.send('mineradio-audio-focus-conflict', payload || {});
  },
});
...
let tray = null;
// Renderer-owned playback snapshot used to keep audio alive when the user
// closes the main window. The audio element remains in the renderer; the main
// process only owns the window/tray lifecycle and never duplicates playback.
...
// Keep app-owned settings and provider credentials next to the executable so
// the Navidrome build behaves as a portable/green app. app.setName() must run
// before the first derived path lookup or Electron can recompute userData.
...
const PORTABLE_USER_DATA_PATH = (() => {
  const override = String(process.env.MINERADIO_PORTABLE_DATA || '').trim();
  if (override && path.isAbsolute(override)) return path.resolve(override);
  // electron-builder's portable target exposes PORTABLE_EXECUTABLE_DIR. For
  // unpacked/dir builds process.execPath is the executable beside userdata.
  const executableRoot = process.env.PORTABLE_EXECUTABLE_DIR
    || (app.isPackaged ? path.dirname(process.execPath) : path.join(__dirname, '..'));
  return path.join(path.resolve(executableRoot), 'userdata');
})();
const LEGACY_USER_DATA_PATH = path.join(app.getPath('appData'), APP_NAME);
const STABLE_USER_DATA_PATH = STARTUP_QA_USER_DATA_PATH || PORTABLE_USER_DATA_PATH;
...
function defaultCacheRootPath() {
  return path.join(STABLE_USER_DATA_PATH, 'cache');
}
...
function isLocalAppUrl(value) {
  try {
    ...
    if (u.protocol === `${APP_SCHEME}:` && u.hostname === 'app') return true;
...
function isTrustedMainDocumentUrl(value) {
  try {
    const u = new URL(String(value || ''));
    if (u.protocol === `${APP_SCHEME}:` && u.hostname === 'app') {
      const appPath = path.posix.normalize(u.pathname || '/');
      return appPath === '/' || appPath === '/index.html';
    }
...
function navidromeErrorResult(error) {
  const raw = String(error && (error.code || error.message) || 'NAVIDROME_OPERATION_FAILED');
  const code = error instanceof NavidromeError || /^(?:NAVIDROME_|SERVER_)/.test(raw)
    ? raw
    : 'NAVIDROME_OPERATION_FAILED';
  return { ok: false, error: code };
}

function navidromeHandler(action) {
  return async (event, ...args) => {
    if (!isTrustedMainWindowIpc(event) || !navidromeService) return { ok: false, error: 'NAVIDROME_UNAVAILABLE' };
    try {
      return { ok: true, value: await action(...args) };
    } catch (error) {
      return navidromeErrorResult(error);
    }
  };
}

ipcMain.handle('navidrome-servers', navidromeHandler(() => navidromeService.servers()));
ipcMain.handle('navidrome-active-server', navidromeHandler(() => navidromeService.activeServer()));
ipcMain.handle('navidrome-verify-server', navidromeHandler((input) => navidromeService.verify(input || {})));
ipcMain.handle('navidrome-save-server', navidromeHandler((input) => navidromeService.save(input || {})));
ipcMain.handle('navidrome-activate-server', navidromeHandler((id) => navidromeService.activate(String(id || ''))));
ipcMain.handle('navidrome-remove-server', navidromeHandler((id) => navidromeService.remove(String(id || ''))));
ipcMain.handle('navidrome-clear-cache', navidromeHandler((id) => navidromeService.clearCache(id ? String(id) : '')));
ipcMain.handle('navidrome-home', navidromeHandler((id) => navidromeService.home(id || '')));
ipcMain.handle('navidrome-songs', navidromeHandler((options) => navidromeService.songs(options || {})));
ipcMain.handle('navidrome-albums', navidromeHandler((options) => navidromeService.albums(options || {})));
ipcMain.handle('navidrome-artists', navidromeHandler((id) => navidromeService.artists(id || '')));
ipcMain.handle('navidrome-artist', navidromeHandler((serverId, id) => navidromeService.artist(serverId || '', String(id || ''))));
ipcMain.handle('navidrome-album', navidromeHandler((serverId, id) => navidromeService.album(serverId || '', String(id || ''))));
ipcMain.handle('navidrome-search', navidromeHandler((serverId, query) => navidromeService.search(serverId || '', String(query || ''))));
ipcMain.handle('navidrome-playlists', navidromeHandler((id) => navidromeService.playlists(id || '')));
ipcMain.handle('navidrome-playlist', navidromeHandler((serverId, id) => navidromeService.playlist(serverId || '', String(id || ''))));
ipcMain.handle('navidrome-favorites', navidromeHandler((id) => navidromeService.favorites(id || '')));
ipcMain.handle('navidrome-lyrics', navidromeHandler((serverId, id) => navidromeService.lyrics(serverId || '', String(id || ''))));
ipcMain.handle('navidrome-media-path', navidromeHandler((serverId, kind, id, maxBitRate) => navidromeService.mediaPath(serverId || '', String(kind || ''), String(id || ''), maxBitRate)));
ipcMain.handle('navidrome-media-info', navidromeHandler((mediaUrl, durationSeconds) => navidromeService.mediaInfo(String(mediaUrl || ''), durationSeconds)));
ipcMain.handle('navidrome-now-playing', navidromeHandler((serverId, id) => navidromeService.nowPlaying(serverId || '', String(id || ''))));
ipcMain.handle('navidrome-scrobble', navidromeHandler((serverId, id) => navidromeService.scrobble(serverId || '', String(id || ''))));
ipcMain.handle('navidrome-star', navidromeHandler((serverId, id) => navidromeService.star(serverId || '', String(id || ''))));
ipcMain.handle('navidrome-unstar', navidromeHandler((serverId, id) => navidromeService.unstar(serverId || '', String(id || ''))));
ipcMain.handle('navidrome-create-playlist', navidromeHandler((serverId, name, songIds) => navidromeService.createPlaylist(serverId || '', String(name || ''), Array.isArray(songIds) ? songIds : [])));
ipcMain.handle('navidrome-add-playlist-songs', navidromeHandler((serverId, playlistId, songIds) => navidromeService.addPlaylistSongs(serverId || '', String(playlistId || ''), Array.isArray(songIds) ? songIds : [])));
ipcMain.handle('navidrome-remove-playlist-songs', navidromeHandler((serverId, playlistId, indexes) => navidromeService.removePlaylistSongs(serverId || '', String(playlistId || ''), Array.isArray(indexes) ? indexes : [])));
ipcMain.handle('navidrome-update-playlist', navidromeHandler((serverId, playlistId, params) => navidromeService.updatePlaylist(serverId || '', String(playlistId || ''), params || {})));
ipcMain.handle('navidrome-delete-playlist', navidromeHandler((serverId, playlistId) => navidromeService.deletePlaylist(serverId || '', String(playlistId || ''))));
ipcMain.handle('navidrome-reorder-playlist', navidromeHandler((serverId, playlistId, songIds) => navidromeService.reorderPlaylist(serverId || '', String(playlistId || ''), Array.isArray(songIds) ? songIds : [])));
...
function updateTrayPlaybackTooltip() {
  if (!tray) return;
  const state = backgroundPlaybackState || {};
  const title = String(state.title || 'Mineradio').trim() || 'Mineradio';
  const artist = String(state.artist || '').trim();
  const suffix = artist ? ` - ${artist}` : '';
  try { tray.setToolTip(`${APP_NAME}: ${title}${suffix}`.slice(0, 127)); } catch (_) {}
}
...
function sendPlaybackCommandToRenderer(command) {
  const action = String(command || '').trim();
  if (!action || !mainWindow || mainWindow.isDestroyed()) return false;
  try {
    mainWindow.webContents.send('mineradio-playback-command', { command: action, at: Date.now() });
    return true;
  } catch (error) {
    console.warn('[TrayPlayback]', action, error && error.message || error);
    return false;
  }
}
...
function createOrUpdateTray() {
  ...
  updateTrayPlaybackTooltip();
  const desktopMode = fullDesktopModeRuntime.getStatus('tray-menu');
  const playbackState = backgroundPlaybackState || {};
  const hasPlaybackTrack = playbackState.hasTrack === true;
  const playPauseLabel = playbackState.playing === true ? '暂停' : '播放/暂停';
  const menu = Menu.buildFromTemplate([
    {
      label: '上一曲',
      enabled: hasPlaybackTrack,
      click: () => sendPlaybackCommandToRenderer('previous'),
    },
    {
      label: playPauseLabel,
      enabled: hasPlaybackTrack,
      click: () => sendPlaybackCommandToRenderer('toggle-play'),
    },
    {
      label: '下一曲',
      enabled: hasPlaybackTrack,
      click: () => sendPlaybackCommandToRenderer('next'),
    },
    { type: 'separator' },
...
ipcMain.handle('mineradio-playback-state', (event, payload = {}) => {
  if (!isTrustedMainWindowIpc(event)) return { ok: false, error: 'PLAYBACK_UNTRUSTED_SENDER' };
  const source = payload && typeof payload === 'object' ? payload : {};
  backgroundPlaybackState = {
    playing: source.playing === true,
    hasTrack: source.hasTrack === true,
    title: String(source.title || 'Mineradio').slice(0, 260),
    artist: String(source.artist || '').slice(0, 320),
    album: String(source.album || '').slice(0, 320),
    lyric: String(source.lyric || '').slice(0, 512),
  };
  if (tray) createOrUpdateTray();
  else updateTrayPlaybackTooltip();
  return { ok: true, backgroundPlayback: closeBehavior === 'tray' };
});

ipcMain.handle('mineradio-audio-focus-configure', (event, payload = {}) => {
  if (!isTrustedMainWindowIpc(event)) return { ok: false, error: 'AUDIO_FOCUS_UNTRUSTED_SENDER' };
  if (payload && payload.enabled === true) return audioFocusMonitor.start();
  return audioFocusMonitor.stop();
});
...
// Portable builds must not inherit the original Mineradio provider sessions.
// Re-encrypt Navidrome secrets in memory because safeStorage ciphertext is
// bound to its Chromium profile directory even for the same Windows account.
async function migrateLegacyNavidromeConfig() {
  // Startup QA explicitly supplies an isolated userData directory. Never
  // import a real profile into that directory, otherwise tests become tied to
  // the developer's personal server configuration.
  if (STARTUP_QA_USER_DATA_PATH) return;
  const sourceDir = path.join(LEGACY_USER_DATA_PATH, 'navidrome');
  const targetDir = path.join(STABLE_USER_DATA_PATH, 'navidrome');
  if (path.resolve(sourceDir) === path.resolve(targetDir) || !fs.existsSync(sourceDir)) return;
  const sourceConfig = path.join(sourceDir, 'servers.json');
  const sourceSecret = path.join(sourceDir, 'server-secrets.json');
  const targetConfig = path.join(targetDir, 'servers.json');
  const targetSecret = path.join(targetDir, 'server-secrets.json');
  if (!fs.existsSync(sourceConfig) || !fs.existsSync(sourceSecret)) return;

  let targetContainsLegacyCiphertext = false;
  try {
    targetContainsLegacyCiphertext = fs.existsSync(targetSecret)
      && fs.readFileSync(targetSecret).equals(fs.readFileSync(sourceSecret));
  } catch (_error) { }
  if (!fs.existsSync(targetSecret) || targetContainsLegacyCiphertext) {
    const localStateCandidates = [
      path.join(LEGACY_USER_DATA_PATH, 'Local State'),
      path.join(LEGACY_USER_DATA_PATH, 'Cache', 'chromium', APP_NAME, 'Local State'),
    ];
    let migrated = false;
    for (const sourceLocalStatePath of localStateCandidates) {
      if (!fs.existsSync(sourceLocalStatePath)) continue;
      try {
        await reencryptLegacyNavidromeSecrets({
          sourceLocalStatePath,
          sourceSecretPath: sourceSecret,
          targetSecretPath: targetSecret,
    ...
    if (!migrated) {
      console.warn('[NavidromeMigration] secret re-encryption skipped');
      return;
    }
  }
  ...
  if (!fs.existsSync(targetConfig)) {
    try {
      ...
      console.log('[NavidromeMigration] restored server profile');
    } catch (error) {
      console.warn('[NavidromeMigration] server profile skipped:', error.message);
    }
  }
}
...
async function ensureLocalServerStarted() {
  ...
  localServerStartPromise = (async () => {
    ...
    await migrateLegacyNavidromeConfig();
    await initializeLoginEasterEggGate();

    const qaCryptoEnabled = process.env.MINERADIO_STARTUP_QA_INSECURE_CRYPTO === '1';
    const qaCrypto = qaCryptoEnabled ? {
      encryptString: (value) => Buffer.from(String(value || ''), 'utf8'),
      decryptString: (value) => Buffer.from(value).toString('utf8'),
    } : null;
    const configStore = new NavidromeConfigStore({
      directory: path.join(STABLE_USER_DATA_PATH, 'navidrome'),
      cryptoAdapter: qaCrypto || {
        encryptString: (value) => safeStorage.encryptString(String(value || '')),
        decryptString: (value) => safeStorage.decryptString(value),
      },
    });
    navidromeService = new NavidromeService({ configStore });
    navidromeService.setMediaBaseUrl(`http://127.0.0.1:${port}`);
    ...
    if (typeof localServer.setNavidromeMediaProxy === 'function') {
      localServer.setNavidromeMediaProxy(navidromeService.mediaProxy);
    }
...
if (!gotSingleInstanceLock) {
  ...
  app.whenReady().then(async () => {
    try {
      await installMineradioAppScheme(protocol, {
        rootDir: path.join(__dirname, '..', 'public'),
        getLocalServerPort: () => mainServerPort,
      });
    } catch (error) {
      console.warn('[MineradioApp] stable app protocol unavailable:', error && error.message || error);
    }
    try {
