...
contextBridge.exposeInMainWorld('desktopWindow', {
  ...
  updatePlaybackState: (payload) => ipcRenderer.invoke('mineradio-playback-state', payload || {}),
  configureAudioFocusPause: (payload) => ipcRenderer.invoke('mineradio-audio-focus-configure', payload || {}),
  onAudioFocusConflict: (callback) => {
    if (typeof callback !== 'function') return () => {};
    const listener = (_event, payload) => callback(payload || {});
    ipcRenderer.on('mineradio-audio-focus-conflict', listener);
    return () => ipcRenderer.removeListener('mineradio-audio-focus-conflict', listener);
  },
  onPlaybackCommand: (callback) => {
    if (typeof callback !== 'function') return () => {};
    const listener = (_event, payload) => callback(payload || {});
    ipcRenderer.on('mineradio-playback-command', listener);
    return () => ipcRenderer.removeListener('mineradio-playback-command', listener);
  },
...
function navidromeInvoke(channel, ...args) {
  return ipcRenderer.invoke(channel, ...args);
}

contextBridge.exposeInMainWorld('navidrome', {
  servers: () => navidromeInvoke('navidrome-servers'),
  activeServer: () => navidromeInvoke('navidrome-active-server'),
  verifyServer: (input) => navidromeInvoke('navidrome-verify-server', input || {}),
  saveServer: (input) => navidromeInvoke('navidrome-save-server', input || {}),
  activateServer: (id) => navidromeInvoke('navidrome-activate-server', String(id || '')),
  removeServer: (id) => navidromeInvoke('navidrome-remove-server', String(id || '')),
  clearCache: (id) => navidromeInvoke('navidrome-clear-cache', id ? String(id) : ''),
  home: (id) => navidromeInvoke('navidrome-home', id || ''),
  songs: (options) => navidromeInvoke('navidrome-songs', options || {}),
  albums: (options) => navidromeInvoke('navidrome-albums', options || {}),
  artists: (id) => navidromeInvoke('navidrome-artists', id || ''),
  artist: (serverId, id) => navidromeInvoke('navidrome-artist', serverId || '', String(id || '')),
  album: (serverId, id) => navidromeInvoke('navidrome-album', serverId || '', String(id || '')),
  search: (serverId, query) => navidromeInvoke('navidrome-search', serverId || '', String(query || '')),
  playlists: (id) => navidromeInvoke('navidrome-playlists', id || ''),
  playlist: (serverId, id) => navidromeInvoke('navidrome-playlist', serverId || '', String(id || '')),
  favorites: (id) => navidromeInvoke('navidrome-favorites', id || ''),
  lyrics: (serverId, id) => navidromeInvoke('navidrome-lyrics', serverId || '', String(id || '')),
  mediaPath: (serverId, kind, id, maxBitRate) => navidromeInvoke('navidrome-media-path', serverId || '', String(kind || ''), String(id || ''), maxBitRate),
  mediaInfo: (mediaUrl, durationSeconds) => navidromeInvoke('navidrome-media-info', String(mediaUrl || ''), durationSeconds),
  nowPlaying: (serverId, id) => navidromeInvoke('navidrome-now-playing', serverId || '', String(id || '')),
  scrobble: (serverId, id) => navidromeInvoke('navidrome-scrobble', serverId || '', String(id || '')),
  star: (serverId, id) => navidromeInvoke('navidrome-star', serverId || '', String(id || '')),
  unstar: (serverId, id) => navidromeInvoke('navidrome-unstar', serverId || '', String(id || '')),
  createPlaylist: (serverId, name, songIds) => navidromeInvoke('navidrome-create-playlist', serverId || '', String(name || ''), Array.isArray(songIds) ? songIds : []),
  addPlaylistSongs: (serverId, playlistId, songIds) => navidromeInvoke('navidrome-add-playlist-songs', serverId || '', String(playlistId || ''), Array.isArray(songIds) ? songIds : []),
  removePlaylistSongs: (serverId, playlistId, indexes) => navidromeInvoke('navidrome-remove-playlist-songs', serverId || '', String(playlistId || ''), Array.isArray(indexes) ? indexes : []),
  updatePlaylist: (serverId, playlistId, params) => navidromeInvoke('navidrome-update-playlist', serverId || '', String(playlistId || ''), params || {}),
  deletePlaylist: (serverId, playlistId) => navidromeInvoke('navidrome-delete-playlist', serverId || '', String(playlistId || '')),
  reorderPlaylist: (serverId, playlistId, songIds) => navidromeInvoke('navidrome-reorder-playlist', serverId || '', String(playlistId || ''), Array.isArray(songIds) ? songIds : []),
});
