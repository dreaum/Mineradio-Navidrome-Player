...
function normalizePlaybackQuality(value) {
  ...
  if (value === 'original' || value === 'raw' || value === 'source') return 'original';
  if (value === '320' || value === '320k') return '320';
  if (value === '192' || value === '192k') return '192';
  if (value === '128' || value === '128k') return '128';
...
function normalizePlaybackQualityForProvider(value, provider) {
  ...
  if (provider === 'navidrome') {
    value = String(value || '').toLowerCase();
    return value === '320' || value === '192' || value === '128' ? value : 'original';
  }
...
function playbackQualityLabel(value, provider) {
  ...
  if (provider === 'navidrome') return value === 'original' ? '原始音质' : ('最高 ' + value + ' kbps');
...
function playbackQualityShortLabel(value, provider) {
  ...
  if (provider === 'navidrome') return value === 'original' ? 'ND RAW' : ('ND ' + value);
...
function playbackQualityRank(value, provider) {
  value = normalizePlaybackQualityForProvider(value, provider);
  if (normalizePlaybackProvider(provider) === 'navidrome') {
    if (value === 'original') return 4;
    if (value === '320') return 3;
...
function scheduleNavidromeQualitySwitchNotice(media, data, song, token) {
  if (
    !media
    || !data
    || !data.url
    || !window.navidrome
    || typeof window.navidrome.mediaInfo !== 'function'
  ) return false;
  var attempts = 0;
  var inFlight = false;
  var finished = false;
  ...
  function invocationIsCurrent() {
    return !finished && token === trackSwitchToken && audio === media;
  }
  function durationSeconds() {
    if (media && isFinite(media.duration) && media.duration > 0) return Number(media.duration);
    var duration = Number(song && (song.duration || song.durationMs || song.dt)) || 0;
    return duration > 1000 ? duration / 1000 : duration;
  }
  function finish(bitRate) {
    if (!invocationIsCurrent()) return;
    finished = true;
    if (fallbackTimer) clearTimeout(fallbackTimer);
    var resolved = Object.assign({}, data);
    if (Number(bitRate) > 0) resolved.br = Number(bitRate);
    if (song && Number(bitRate) > 0) song.playbackBitRate = Number(bitRate);
    var requested = String(resolved.level || 'original').toLowerCase();
    var cap = requested === '320' || requested === '192' || requested === '128' ? Number(requested) : 0;
    var measured = Number(bitRate) || 0;
    // Navidrome may keep the source representation when transcoding is disabled.
    // Flag that case instead of presenting the selected cap as if it were real.
    var exceeded = cap > 0 && measured > cap * 1000 * 1.15;
    if (exceeded) {
      showSourceFallbackNotice(
        '服务端未按上限转码',
        '请求最高 ' + cap + ' kbps，实际播放: ' + playbackBitrateLabel(measured) + '。请检查 Navidrome 转码配置。'
      );
    } else {
      showSourceFallbackNotice('音质已切换', '实际播放: ' + playbackResolvedQualityText(resolved, 'navidrome') + '。');
    }
  ...
  function retry() {
    ...
    fallbackTimer = setTimeout(probe, 140);
  }
  function probe() {
    if (!invocationIsCurrent() || inFlight) return;
    ...
    attempts++;
    inFlight = true;
    Promise.resolve(window.navidrome.mediaInfo(data.url, duration)).then(function (result) {
      inFlight = false;
      if (!invocationIsCurrent()) return;
      var info = result && result.ok ? result.value : null;
      if (info && info.ready && Number(info.bitRate) > 0) {
        finish(info.bitRate);
        return;
      }
      retry();
  ...
  media.addEventListener('loadedmetadata', probe, { once: true });
  fallbackTimer = setTimeout(probe, media.readyState >= 1 ? 0 : 220);
  return true;
}
...
function readPlaybackQualityPreference() {
  var fallback = {
    ...
    navidrome: PLAYBACK_QUALITY_DEFAULTS.navidrome
  };
  try {
    ...
    if (raw.trim().charAt(0) !== '{') {
      ...
      return {
        ...
        navidrome: normalizePlaybackQualityForProvider(fallback.navidrome, 'navidrome')
      };
    }
    ...
    return {
      ...
      navidrome: normalizePlaybackQualityForProvider(parsed.navidrome || fallback.navidrome || 'original', 'navidrome')
    };
  } catch (e) {
    return fallback;
  }
...
function updatePlaybackQualityUi() {
  ...
  var qualityProviderTitle = provider === 'navidrome' ? 'Navidrome 音质: ' : (provider === 'spotify' ? 'Spotify 匹配源: ' : (provider === 'qishui' ? '汽水音质: ' : (provider === 'qq' ? 'QQ 音质: ' : (provider === 'kugou' ? '酷狗音质: ' : '网易云音质: '))));
...
function canReloadCurrentTrackForQuality() {
  ...
  return songProviderKey(song) === 'netease' || songProviderKey(song) === 'qq' || songProviderKey(song) === 'kugou' || songProviderKey(song) === 'navidrome';
}
