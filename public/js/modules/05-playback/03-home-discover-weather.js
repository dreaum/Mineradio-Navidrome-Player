function fallbackHomeTiles() {
  return [
    { kind: 'login', title: '登录同步歌单', sub: 'Navidrome / 网易云 / QQ / 酷狗 / 汽水' },
...
function renderHomeTiles() {
  ...
  if (!row) return;
  var tiles = [];
  var loggedOutHome = !homeDiscoverState.loggedIn && !hasAnyConnectedMusicSource();
...
function renderHomeDiscover() {
  ...
  if (weatherMeta) {
    var meta = loggedOutHome
      ? ['Navidrome 曲库', '跨平台搜索', '本地音乐']
      : ['Navidrome 曲库', '平台歌单', '热门电台'];
...
async function loadHomeDiscover(force) {
  ...
  renderHomeDiscover();
  try {
    // A connected Navidrome server is a first-class library source.  Do not let
    // an unrelated legacy platform login hide it from the Mineradio home view.
    var navidromeConnected = typeof navidromeIsConnected === 'function' && navidromeIsConnected();
    var data = navidromeConnected
      ? await navidromeHomeDiscoverData(!!force)
      : await apiJson('/api/discover/home?t=' + Date.now());
    if (token !== homeDiscoverToken) return;
    homeDiscoverState.loggedIn = !!(data && data.loggedIn) || hasAnyConnectedMusicSource();
