...
async function togglePlay() {
  ...
  try {
    ...
    if (audio.paused || audio.ended) {
      ...
      if (typeof audioFocusPauseByExternalMedia !== 'undefined') audioFocusPauseByExternalMedia = false;
...
function nextTrack(userInitiated) {
  ...
  if (currentIdx >= playQueue.length - 1 && queueHydrationState && queueHydrationState.queueRef === playQueue && (queueHydrationState.active || queueHydrationState.loading) && !queueHydrationState.error) {
    ...
    return Promise.resolve(hydratePlaylistQueueNextPage('queue-tail')).then(function () {
      ...
      console.warn('[PlaybackNavigation] next queue hydration failed:', error);
      showToast('后续歌曲载入失败，当前歌曲保持不变');
      return false;
  ...
  return Promise.resolve(playQueueAt(currentIdx, opts)).catch(function (error) {
    console.warn('[PlaybackNavigation] next track failed:', error);
    return false;
  }).finally(forcePlaybackControlsInteractive);
}
function prevTrack(userInitiated) {
  ...
  if (playMode === 'shuffle') opts.skipShuffleOrder = true;
  return Promise.resolve(playQueueAt(currentIdx, opts)).catch(function (error) {
    console.warn('[PlaybackNavigation] previous track failed:', error);
    return false;
  }).finally(forcePlaybackControlsInteractive);
}
