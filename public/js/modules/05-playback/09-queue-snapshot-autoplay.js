...
function queueItemKey(song) {
  if (!song) return '';
  if (song.type === 'navidrome' || song.provider === 'navidrome' || song.source === 'navidrome') return 'navidrome:' + String(song.serverId || '') + ':' + String(song.id || (song.name + '|' + song.artist));
...
function playbackRestoreSongSnapshot(song) {
  ...
  var snap = {};
  [
    'provider', 'source', 'type', 'serverId', 'id', 'mid', 'songmid', 'mediaMid', 'media_mid', 'qqId',
    ...
    'name', 'title', 'artist', 'album', 'cover', 'coverArt', 'artistId', 'albumId', 'starred', 'bitRate', 'duration', 'durationMs', 'dt', 'fee',
...
function saveLastPlaybackSnapshot(force, reason) {
  ...
  if (songProviderKey(song) === 'navidrome') {
    saveNavidromePlaybackSession();
    try { localStorage.removeItem(LAST_PLAYBACK_STORE_KEY); } catch (_error) {}
...
function restoreLastPlaybackSnapshot() {
  ...
  if (songProviderKey(snapshot.current) === 'navidrome') {
    try { localStorage.removeItem(LAST_PLAYBACK_STORE_KEY); } catch (_error) {}
    return false;
  }
...
function tryStartupAutoplayHomeFallback(jobId) {
  ...
  if (!hasAnyConnectedMusicSource()) return false;
