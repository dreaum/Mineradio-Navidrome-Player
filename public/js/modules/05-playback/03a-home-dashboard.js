...
function renderHomeDashboardQuickCards() {
  ...
  var navidromeConnected = typeof navidromeIsConnected === 'function' && navidromeIsConnected();
  var navidromeConfigured = typeof navidromeHasConfiguredServer === 'function' && navidromeHasConfiguredServer();
  var navidromeProfile = navidromeConnected && typeof navidromeCurrentServerProfile === 'function' ? navidromeCurrentServerProfile() : null;
  ...
  var cards = [
    ...
    },
    {
      label: 'LIBRARY',
      title: '音乐库',
      sub: navidromeConnected
        ? ((navidromeProfile && (navidromeProfile.name || navidromeProfile.url) || '当前服务器') + ' · 服务器歌曲与歌单')
        : (navidromeConfigured ? 'Navidrome 已配置 · 请重新输入密码后加载服务器歌曲与歌单'
          : (libraryCount ? (libraryCount + ' 项内容 · 本地音乐与歌单') : '歌单、本地音乐和已登录平台')),
      cover: navidromeConnected && homeDiscoverState && homeDiscoverState.songs && homeDiscoverState.songs[0]
        ? homeDashboardSongCover(homeDiscoverState.songs[0], 260)
        : (localSongs[0] ? homeDashboardSongCover(localSongs[0], 260) : ''),
...
function renderHomeDashboardDiscovery() {
  ...
  if (!homeDashboardDiscoveryCache.length) {
    root.innerHTML = '<button class="home-discovery-empty" type="button" onclick="openHomeDashboardLibrary()">' +
      '<strong>等待你的音乐</strong><span>' + ((typeof navidromeIsConnected === 'function' && navidromeIsConnected()) ? '当前服务器暂无可展示歌曲' : '登录平台或导入本地音乐后生成推荐') + '</span></button>';
    return;
  }
...
function openHomeDashboardLibrary() {
  var loggedIn = typeof hasAnyConnectedMusicSource === 'function' && hasAnyConnectedMusicSource()
...
function openHomeDashboardCharts() {
  openHomePlatformRecommendations(typeof navidromeIsConnected === 'function' && navidromeIsConnected() ? 'navidrome' : 'netease');
}
...
function homePlatformNeteaseData() {
  var navidromeConnected = typeof navidromeIsConnected === 'function' && navidromeIsConnected();
  if (navidromeConnected) {
    return homePlatformRecommendationState.neteaseData || {
      loading: true, loaded: false, loggedIn: false, songs: [], playlists: [], podcasts: [], error: ''
    };
  }
  return homeDiscoverState || { loading: false, loggedIn: false, songs: [], playlists: [], podcasts: [], error: '' };
}
...
function renderHomePlatformDailyWindow(force) {
  ...
  if (!list || !grid) return;
  var neteaseData = homePlatformNeteaseData();
  var songs = Array.isArray(neteaseData.songs) ? neteaseData.songs : [];
...
function renderHomePlatformRecommendations() {
  ...
  if (source === 'netease') {
    var neteaseData = homePlatformNeteaseData();
    if (neteaseData.loading || homePlatformRecommendationState.neteaseLoading) {
      status.textContent = '正在读取网易云平台推荐…';
      list.innerHTML = '<div class="home-platform-recommend-loading">正在同步推荐内容</div>';
      return;
    }
    var sections = [];
    var playlists = Array.isArray(neteaseData.playlists) ? neteaseData.playlists.slice(0, 6) : [];
    var songs = Array.isArray(neteaseData.songs) ? neteaseData.songs : [];
    if (playlists.length) {
    ...
    if (sections.length) {
      ...
      status.textContent = neteaseData.error ? '网易云推荐读取失败' : '网易云暂未返回推荐内容';
      status.classList.toggle('is-error', !!neteaseData.error);
      list.innerHTML = homePlatformRecommendationEmptyHtml('netease', neteaseData.loggedIn
  ...
  if (source === 'navidrome') {
    if (typeof navidromeIsConnected !== 'function' || !navidromeIsConnected()) {
      status.textContent = '尚未连接 Navidrome 服务器';
      list.innerHTML = homePlatformRecommendationEmptyHtml('navidrome', '从右上角账号入口添加并验证服务器后，这里会显示服务器歌单和曲库内容。');
      return;
    }
    if (homeDiscoverState.loading) {
      status.textContent = '正在读取 Navidrome 服务器曲库…';
      list.innerHTML = '<div class="home-platform-recommend-loading">正在同步服务器内容</div>';
      return;
    }
    var navidromeSections = [];
    var navidromePlaylists = Array.isArray(homeDiscoverState.playlists) ? homeDiscoverState.playlists.slice(0, 12) : [];
    var navidromeSongs = Array.isArray(homeDiscoverState.songs) ? homeDiscoverState.songs.slice(0, 24) : [];
    if (navidromePlaylists.length) {
      navidromeSections.push('<section><h3>服务器歌单</h3><div class="home-platform-recommend-grid">' + navidromePlaylists.map(function (item, index) {
        return homePlatformRecommendationCard('navidrome-playlist', index, item, 'Navidrome 服务器歌单');
      }).join('') + '</div></section>');
    }
    if (navidromeSongs.length) {
      navidromeSections.push('<section><h3>服务器曲库</h3><div class="home-platform-recommend-grid">' + navidromeSongs.map(function (item, index) {
        return homePlatformRecommendationCard('navidrome-song', index, item, 'Navidrome 曲库');
      }).join('') + '</div></section>');
    }
    if (navidromeSections.length) {
      status.textContent = '来自当前 Navidrome 服务器；完整歌曲在“音乐库”中按需加载。';
      list.innerHTML = navidromeSections.join('');
    } else {
      status.textContent = homeDiscoverState.error ? 'Navidrome 内容读取失败' : '服务器暂未返回可展示的歌单或歌曲';
      status.classList.toggle('is-error', !!homeDiscoverState.error);
      list.innerHTML = homePlatformRecommendationEmptyHtml('navidrome', homeDiscoverState.error
        ? '请刷新后重试，或检查服务器访问权限。'
        : '服务器歌单和曲库会在连接成功后显示在这里。');
    }
    return;
  }
...
async function loadHomePlatformNeteaseRecommendations(force) {
  ...
  try {
    ...
    if (navidromeConnected) {
      ...
      homePlatformRecommendationState.neteaseData = {
        loading: false,
        loaded: true,
        loggedIn: !!(data && data.loggedIn),
        mode: data && data.mode || 'starter',
        songs: !!(data && data.loggedIn) ? (data.dailySongs || []).map(cloneSong) : [],
        playlists: !!(data && data.loggedIn) ? (data.playlists || []) : [],
        podcasts: !!(data && data.loggedIn) ? (data.podcasts || []) : [],
        error: ''
      };
    } else {
    ...
    var neteaseData = homePlatformNeteaseData();
    if (force || !Array.isArray(neteaseData.podcasts) || !neteaseData.podcasts.length) {
      ...
      if (hotPodcasts.length) neteaseData.podcasts = hotPodcasts;
    }
  } catch (error) {
    console.warn('[HomePlatformNetease]', error);
    if (typeof navidromeIsConnected === 'function' && navidromeIsConnected()) {
      homePlatformRecommendationState.neteaseData = Object.assign({}, homePlatformRecommendationState.neteaseData || {}, {
        loading: false, loaded: true, songs: [], playlists: [], podcasts: [], error: 'DISCOVER_FAILED'
      });
    }
  } finally {
...
function playHomePlatformNeteaseSong(index) {
  var data = homePlatformNeteaseData();
  ...
  Promise.resolve(playQueueAt(currentIdx, {
    manual: true,
    context: { type: 'home-platform-recommendation', playlistName: '网易云推荐' },
  })).catch(function (error) { console.warn('[HomePlatformNeteasePlay]', error); });
}

function openHomePlatformNeteasePlaylist(index) {
  var data = homePlatformNeteaseData();
  var playlists = Array.isArray(data.playlists) ? data.playlists : [];
  var item = playlists[Number(index) || 0];
  if (!item || !item.id) return;
  homeForcedOpen = false;
  homeSuppressed = false;
  if (typeof setHomeControlsLocked === 'function') setHomeControlsLocked(false);
  Promise.resolve(loadPlaylistIntoQueueById(playlistPanelProviderId('netease', item.id), true, item.name || '网易云推荐'))
    .catch(function (error) { console.warn('[HomePlatformNeteasePlaylist]', error); });
}
...
async function loadHomePlatformRecommendations(source, force) {
  ...
  try {
    ...
    } else if (homePlatformRecommendationState.source === 'navidrome') {
...
function bindHomePlatformRecommendationControls() {
  ...
  if (list) list.addEventListener('click', function (event) {
    ...
    if (kind === 'netease-playlist') openHomePlatformNeteasePlaylist(index);
    else if (kind === 'netease-song') playHomePlatformNeteaseSong(index);
    else if (kind === 'navidrome-playlist' && typeof openHomePlaylist === 'function') openHomePlaylist(index);
    else if (kind === 'navidrome-song' && typeof playHomeSong === 'function') playHomeSong(index);
