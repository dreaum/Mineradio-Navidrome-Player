...
function currentArtistId(song) {
  if (!song) return '';
  if (songProviderKey(song) === 'navidrome') return String(song.artistId || '');
...
function currentAlbumKey(song) {
  ...
  if (provider === 'navidrome') {
    var navidromeAlbumId = song.albumId || song.album_id || '';
    return navidromeAlbumId ? 'navidrome:' + String(song.serverId || '') + ':' + navidromeAlbumId : '';
  }
  return '';
}
...
function albumDetailMissingText(song) {
  var provider = songProviderKey(song);
  if (provider === 'navidrome') return '当前歌曲缺少 Navidrome 专辑 ID。';
...
function openTrackDetailModal(type, songOverride) {
  ...
  if (type === 'album') {
    var isNavidromeAlbum = songProviderKey(song) === 'navidrome';
    var albumUrl = isNavidromeAlbum && song.albumId ? 'navidrome' : albumDetailUrlForSong(song);
    ...
    if (albumUrl) {
      var albumRequest = isNavidromeAlbum ? navidromeAlbumDetailForSong(song) : apiJson(albumUrl);
      Promise.resolve(albumRequest).then(function (r) {
        if (seq !== trackDetailSeq) return;
    ...
    var artistDetailUrl = isNavidromeArtist && artistId
      ? 'navidrome'
      : artistId
    ...
    var artistEmptyText = songProviderKey(song) === 'qq'
      ...
      : '当前歌曲缺少可用的歌手主页信息';
    var artistLoadingText = isNavidromeArtist ? '正在载入 Navidrome 歌手资料...' : (songProviderKey(song) === 'qq' ? '正在载入 QQ 歌手主页...' : '正在载入歌手主页...');
    ...
    if (artistDetailUrl) {
      var artistRequest = isNavidromeArtist ? navidromeArtistDetailForSong(song) : apiJson(artistDetailUrl);
      Promise.resolve(artistRequest).then(function (r) {
        if (seq !== trackDetailSeq) return;
...
function songAccountStateKey(song) {
  ...
  if (provider === 'navidrome' && id) return provider + ':' + String(song && song.serverId || '') + ':' + id;
...
function playlistAccountProvider(playlist) {
  ...
  return /^(netease|qq|kugou|qishui|spotify|navidrome)$/.test(provider) ? provider : 'netease';
}
function songAccountLoginStatus(provider) {
  if (provider === 'navidrome') return { loggedIn: navidromeIsConnected() };
...
async function toggleLikeSong(song) {
  ...
  var adapter = songAccountAdapter(provider);
  if (provider === 'navidrome') {
    if (!ensureLoggedInForAction(provider) || !song || !song.id) return;
    var navKey = songAccountStateKey(song);
    if (likeBusyMap[navKey]) return;
    var navNext = !isSongLiked(song);
    likeBusyMap[navKey] = true;
    likedSongMap[navKey] = navNext;
    song.starred = navNext;
    updateLikeButtons(song);
    safeRenderQueuePanel('navidrome-like-optimistic', { scrollCurrent: miniQueueOpen });
    refreshSearchResultActionStates();
    try {
      var navResult = navNext
        ? await window.navidrome.star(song.serverId, song.id)
        : await window.navidrome.unstar(song.serverId, song.id);
      if (!navResult || !navResult.ok) throw new Error(navResult && navResult.error || 'NAVIDROME_FAVORITE_FAILED');
      if (window.NavidromeStore && typeof window.NavidromeStore.invalidateMetadata === 'function') {
        window.NavidromeStore.invalidateMetadata(song.serverId).then(function () {
          if (typeof navidromeSyncProviderState === 'function') return navidromeSyncProviderState(true);
        }).catch(function () {});
      }
      ...
      likedSongMap[navKey] = !navNext;
      song.starred = !navNext;
      showToast('Navidrome 收藏同步失败');
    } finally {
      delete likeBusyMap[navKey];
      updateLikeButtons(song);
...
function openCollectModal(song) {
  ...
  if (provider === 'navidrome') {
    ...
    refreshUserPlaylists(true).then(renderCollectModal).catch(renderCollectModal);
    return;
  }
...
async function createPlaylistFromCollect() {
  ...
  if (provider === 'navidrome') {
    if (!ensureLoggedInForAction(provider)) return;
    var navInput = document.getElementById('collect-new-name');
    var navName = navInput ? navInput.value.trim() : '';
    if (!navName) { showToast('先输入歌单名称'); return; }
    try {
      var navServerId = collectTargetSong && collectTargetSong.serverId || navidromeCurrentServerId();
      var navCreated = await window.navidrome.createPlaylist(navServerId, navName, []);
      if (!navCreated || !navCreated.ok) throw new Error(navCreated && navCreated.error || 'NAVIDROME_PLAYLIST_CREATE_FAILED');
      if (navInput) navInput.value = '';
      if (window.NavidromeStore && typeof window.NavidromeStore.invalidateMetadata === 'function') await window.NavidromeStore.invalidateMetadata(navServerId);
      await refreshUserPlaylists(true);
      renderCollectModal();
      var createdId = navCreated.value && (navCreated.value.id || navCreated.value.playlistId);
      if (createdId && collectTargetSong) addCollectTargetToPlaylist(createdId);
      else showToast('Navidrome 歌单已创建');
    } catch (_error) { showToast('创建 Navidrome 歌单失败'); }
    return;
  }
...
async function addCollectTargetToPlaylist(pid) {
  ...
  if (provider === 'navidrome') {
    ...
    updateLikeButtons();
    try {
      var navAdded = await window.navidrome.addPlaylistSongs(targetSong.serverId || navidromeCurrentServerId(), pid, [targetSong.id]);
      if (!navAdded || !navAdded.ok) throw new Error(navAdded && navAdded.error || 'NAVIDROME_PLAYLIST_ADD_FAILED');
      if (window.NavidromeStore && typeof window.NavidromeStore.invalidateMetadata === 'function') await window.NavidromeStore.invalidateMetadata(targetSong.serverId || navidromeCurrentServerId());
      if (typeof navidromeSyncProviderState === 'function') await navidromeSyncProviderState(true);
      showToast('已加入 Navidrome 歌单');
      closeCollectModal();
    } catch (_error) { showToast('加入 Navidrome 歌单失败'); }
    finally {
      collectBusy = false;
