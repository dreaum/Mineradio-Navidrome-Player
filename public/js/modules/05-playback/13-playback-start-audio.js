...
async function playQueueAt(idx, opts) {
  ...
  try {
    ...
    try {
      ...
      var isNavidromePlayback = playbackProvider === 'navidrome';
      ...
      } else if (isNavidromePlayback) {
        var navidromeMediaResult = await window.navidrome.mediaPath(
          song.serverId,
          'stream',
          song.id,
          navidromeMaxBitRateForQuality(requestedQuality)
        );
        if (!navidromeMediaResult || !navidromeMediaResult.ok || !navidromeMediaResult.value) {
          throw new Error(navidromeMediaResult && navidromeMediaResult.error || 'NAVIDROME_MEDIA_PATH_FAILED');
        }
        data = {
          url: String(navidromeMediaResult.value),
          provider: 'navidrome',
      ...
      } else if (!opts.startupAutoplay && opts.qualitySwitch && !isNavidromePlayback) {
      ...
      var proxyAudioUrl = opts.preloadedProxyAudioUrl || (data.directMediaUrl ? data.url : '/api/audio?url=' + encodeURIComponent(data.url));
      if (albumGaplessHandoff) {
        audioFadeSerial++;
        clearAudioFadeTimers();
      ...
      if (
        ...
        && opts.qualitySwitch
        && isNavidromePlayback
        && typeof scheduleNavidromeQualitySwitchNotice === 'function'
      ) {
        scheduleNavidromeQualitySwitchNotice(audio, data, song, token);
      }
      ...
      forcePlaybackControlsInteractive();
      if (isNavidromePlayback) navidromeStartPlaybackReporting(song, audio, token);
      if (playbackProvider === 'netease' && data && data.sourceMatch) {
