...
var SEARCH_HISTORY_MODES = ['song', 'netease', 'qq', 'kugou', 'qishui', 'spotify', 'navidrome', 'podcast'];
...
function resetSearchMusicRenderState() {
  ...
  searchMusicRenderState.albums = [];
  searchMusicRenderState.artists = [];
...
function updateSearchModeTabs() {
  ...
  if ($input && searchMode === 'navidrome') $input.placeholder = '搜索 Navidrome 曲库...';
  requestAnimationFrame(updateSearchPillGlassDisplacementMap);
}
function setSearchMode(mode) {
  mode = (mode === 'podcast' || mode === 'netease' || mode === 'qq' || mode === 'kugou' || mode === 'qishui' || mode === 'spotify' || mode === 'navidrome') ? mode : 'song';
...
$results.addEventListener('click', function (e) {
  var facet = e.target && e.target.closest ? e.target.closest('[data-search-facet]') : null;
  if (facet) {
    e.preventDefault();
    e.stopPropagation();
    openNavidromeSearchFacet(facet.getAttribute('data-search-facet'), Number(facet.getAttribute('data-search-facet-index')));
    return;
  }
...
function songProviderKey(song) {
  if (song && (song.provider === 'navidrome' || song.source === 'navidrome' || song.type === 'navidrome' || song.serverId && song.coverArt)) return 'navidrome';
...
function songSourceTagHtml(song, opts) {
  ...
  var key = /^(netease|qq|kugou|qishui|spotify|navidrome)$/.test(String(rawKey || '')) ? String(rawKey) : songProviderKey(song);
  var label = key === 'qq' ? 'QQ' : (key === 'kugou' ? 'KG' : (key === 'qishui' ? 'QS' : (key === 'spotify' ? 'SP' : (key === 'navidrome' ? 'ND' : 'NE'))));
  if (opts.switcher) {
...
function controlSourceProviders() {
  ...
  // Keep a configured-but-disconnected Navidrome source visible so the
  // original player can explain how to repair it instead of hiding it.
  var navidromeVisible = typeof navidromeHasConfiguredServer === 'function'
    ? navidromeHasConfiguredServer()
    : (typeof navidromeIsConnected === 'function' && navidromeIsConnected());
  if (navidromeVisible) {
    providers.push({ key: 'navidrome', label: 'ND', title: 'Navidrome' });
  }
  return providers;
}
...
function renderControlSourceSwitcher(matches) {
  ...
  el.innerHTML =
    ...
    controlSourceProviders().map(function (provider) {
      ...
      var navidromeUnavailable = provider.key === 'navidrome'
        && typeof navidromeIsConnected === 'function'
        && !navidromeIsConnected();
      var providerLimited = !!(match && provider.key === 'spotify' && match.playable === false);
      var unavailableHint = navidromeUnavailable && typeof navidromeConnectionHint === 'function' ? navidromeConnectionHint() : '';
      var cleanStatus = active ? '当前' : (navidromeUnavailable ? unavailableHint : (providerLimited ? '匹配源' : (match ? '可切换' : (controlSourceSwitcherState.loading ? '检测中' : controlSourceIssueLabel(issue)))));
      var title = active ? '当前音源' : (navidromeUnavailable ? unavailableHint : (providerLimited ? (provider.title + ': 播放将自动换源') : (match ? ('切换到 ' + provider.title) : (provider.title + ': ' + controlSourceIssueLabel(issue)))));
      var status = active ? '当前' : (navidromeUnavailable ? '未连接' : (providerLimited ? '匹配源' : (match ? '可切换' : (controlSourceSwitcherState.loading ? '检测中' : '无匹配'))));
...
async function findControlSourceMatchResult(song, provider) {
  ...
  var data = provider === 'navidrome'
    ? await navidromeSearchSongs(query)
    : await apiJson(controlSourceSearchUrl(provider, query), { timeoutMs: 6000 });
  var list = provider === 'navidrome' ? data : (data && (data.songs || data.result || []));
...
var MUSIC_SEARCH_PROVIDER_ORDER = ['navidrome', 'netease', 'qq', 'kugou', 'qishui', 'spotify'];
function searchProviderStatus(provider) {
  if (provider === 'navidrome') return { loggedIn: navidromeIsConnected(), searchReady: navidromeIsConnected(), publicCatalog: navidromeIsConnected() };
...
function searchModeProvider(mode) {
  return mode === 'netease' || mode === 'qq' || mode === 'kugou' || mode === 'qishui' || mode === 'spotify' || mode === 'navidrome' ? mode : '';
}
...
function searchQueryTokens(text) {
  ...
  return raw.split(/[\s·・，。,.!?！？"“”‘’()（）【】\[\]\-_/]+/).map(function (token) {
    ...
    if (!token || /^(qq|网易云|网易云音乐|酷狗|酷狗音乐|汽水|汽水音乐|spotify|navidrome|nd)$/.test(token)) return false;
...
function mergeSongSearchResults(neteaseSongs, qqSongs, kugouSongs, qishuiSongs, spotifySongs, navidromeSongs, limit, q) {
  var out = [];
  var providerSeen = {};
  var canonicalSeen = {};
  ...
  // The connected personal library wins canonical de-duplication so an
  // available server copy is not hidden behind a public catalogue result.
...
async function fetchMusicSearchResults(q, mode, previousPages) {
  ...
  var pageLimitByProvider = { netease: 18, qq: 12, kugou: 12, qishui: 12, spotify: 10, navidrome: 60 };
  ...
  var result = await Promise.allSettled(fetchProviders.map(function (provider) {
    ...
    var request = provider === 'navidrome' ? navidromeSearchCatalog(q) : apiJson(searchProviderUrl(provider, q, limit, offset));
    return Promise.resolve(request).then(function (value) {
      if (provider === 'navidrome') value = {
        songs: value && value.song || [],
        albums: value && value.album || [],
        artists: value && value.artist || [],
        total: value && value.song && value.song.length || 0,
        hasMore: false,
        nextOffset: value && value.song && value.song.length || 0
      };
      return { provider: provider, offset: offset, requestedLimit: limit, value: value || {} };
    });
  }));
  var songsByProvider = { netease: [], qq: [], kugou: [], qishui: [], spotify: [], navidrome: [] };
  var navidromeFacets = { albums: [], artists: [] };
  fetchProviders.forEach(function (provider, index) {
    ...
    if (provider === 'navidrome') {
      navidromeFacets.albums = Array.isArray(value.albums) ? value.albums : [];
      navidromeFacets.artists = Array.isArray(value.artists) ? value.artists : [];
    }
  ...
  var songs = mergeSongSearchResults(
    ...
    songsByProvider.navidrome,
    MUSIC_SEARCH_MAX_RESULTS,
    q
  );
  return { songs: songs, providerPages: providerPages, hasMore: searchProviderPagesHaveMore(providerPages), facets: navidromeFacets };
}
function navidromeSearchFacetCover(item) {
  item = item || {};
  return String(item.cover || item.coverPath || '');
}
function navidromeSearchFacetRow(item, type, index) {
  item = item || {};
  var cover = navidromeSearchFacetCover(item);
  var img = cover
    ? '<img src="' + escHtml(coverUrlWithSize(cover, 80)) + '" alt="" loading="lazy" onerror="this.style.opacity=0.2">'
    : '<div class="search-facet-placeholder">' + escHtml(type === 'artist' ? String(item.name || '歌').slice(0, 1) : 'AL') + '</div>';
  var meta = type === 'artist'
    ? ((Number(item.albumCount) || 0) ? Number(item.albumCount) + ' 张专辑' : 'Navidrome 歌手')
    : String(item.artist || '未知歌手');
  return '<div class="search-result navidrome-source search-facet-result" data-search-facet="' + type + '" data-search-facet-index="' + index + '">' +
    img + '<div class="search-result-info"><div class="search-result-title">' + escHtml(item.name || (type === 'artist' ? '未知歌手' : '未知专辑')) + '<span class="tag-source navidrome">ND</span></div>' +
    '<div class="search-result-meta">' + escHtml(meta) + '</div></div><span class="search-facet-arrow">›</span></div>';
}
function navidromeSearchGroupHtml(title, type, items) {
  items = Array.isArray(items) ? items.slice(0, 4) : [];
  if (!items.length) return '';
  return '<div class="search-result-group" data-search-group="' + type + '"><div class="search-history-head"><span>' + title + '</span></div>' +
    items.map(function (item, index) { return navidromeSearchFacetRow(item, type, index); }).join('') + '</div>';
}
function navidromeSearchSongGroupHead() {
  return '<div class="search-history-head search-result-group-head"><span>歌曲</span></div>';
}
function openNavidromeSearchFacet(type, index) {
  var items = type === 'artist' ? searchMusicRenderState.artists : searchMusicRenderState.albums;
  var item = items && items[index];
  if (!item) return;
  var seed = {
    type: 'navidrome', provider: 'navidrome', source: 'navidrome',
    serverId: item.serverId || navidromeCurrentServerId(),
    name: item.name || (type === 'artist' ? '未知歌手' : '未知专辑'),
    title: item.name || '', artist: item.artist || item.name || '未知歌手',
    artistId: type === 'artist' ? item.id : item.artistId,
    album: type === 'album' ? item.name : '', albumId: type === 'album' ? item.id : '',
    cover: navidromeSearchFacetCover(item), coverArt: item.coverArt || ''
  };
  openTrackDetailModal(type, seed);
}
...
async function loadNextMusicSearchPage(expectedKey) {
  ...
  try {
    ...
    if (page.facets) {
      searchMusicRenderState.albums = page.facets.albums || searchMusicRenderState.albums;
      searchMusicRenderState.artists = page.facets.artists || searchMusicRenderState.artists;
    }
    playlist = merged;
...
function renderSongSearchResults(songs) {
  ...
  searchMusicRenderState.albums = plan.facets && plan.facets.albums || [];
  searchMusicRenderState.artists = plan.facets && plan.facets.artists || [];
  searchMusicRenderState.visibleCount = Math.min(playlist.length, MUSIC_SEARCH_INITIAL_VISIBLE);
  var html = navidromeSearchGroupHtml('艺术家', 'artist', searchMusicRenderState.artists) +
    navidromeSearchGroupHtml('专辑', 'album', searchMusicRenderState.albums) +
    navidromeSearchSongGroupHead();
...
async function doSearch(q, opts) {
  ...
  try {
    ...
    pendingSearchProviderPages = {
      ...
      facets: searchData.facets || { albums: [], artists: [] },
