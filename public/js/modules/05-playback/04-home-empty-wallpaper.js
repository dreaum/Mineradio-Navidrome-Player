...
var homePlayerSafeAreaRaf = 0;
function homePlayerBaseSafeBottom() {
  if (window.innerWidth <= 620) return 44;
  if (window.innerHeight <= 700) return 34;
  if (window.innerHeight <= 760) return 44;
  return 58;
}
function updateHomePlayerSafeArea() {
  var bar = document.getElementById('bottom-bar');
  var visible = !!(bar && bar.classList.contains('visible') && !bar.classList.contains('soft-hidden') && !document.body.classList.contains('home-controls-locked'));
  var safeBottom = homePlayerBaseSafeBottom();
  if (visible) {
    var rect = bar.getBoundingClientRect();
    if (rect.height > 0) safeBottom = Math.max(safeBottom, Math.ceil(window.innerHeight - rect.top + 8));
  }
  document.documentElement.style.setProperty('--mineradio-home-safe-bottom', safeBottom + 'px');
  return safeBottom;
}
function scheduleHomePlayerSafeAreaSync() {
  if (homePlayerSafeAreaRaf) cancelAnimationFrame(homePlayerSafeAreaRaf);
  homePlayerSafeAreaRaf = requestAnimationFrame(function () {
    homePlayerSafeAreaRaf = 0;
    updateHomePlayerSafeArea();
  });
}
(function bindHomePlayerSafeAreaSync() {
  var bar = document.getElementById('bottom-bar');
  if (window.MutationObserver) {
    var observer = new MutationObserver(scheduleHomePlayerSafeAreaSync);
    if (bar) observer.observe(bar, { attributes: true, attributeFilter: ['class', 'style'] });
    observer.observe(document.body, { attributes: true, attributeFilter: ['class'] });
  }
  if (window.ResizeObserver && bar) new ResizeObserver(scheduleHomePlayerSafeAreaSync).observe(bar);
  window.addEventListener('resize', scheduleHomePlayerSafeAreaSync, { passive: true });
  ['(max-height:760px)', '(max-height:700px)', '(max-width:760px)', '(max-width:620px)'].forEach(function (query) {
    var media = window.matchMedia && window.matchMedia(query);
    if (!media) return;
    if (media.addEventListener) media.addEventListener('change', scheduleHomePlayerSafeAreaSync);
    else if (media.addListener) media.addListener(scheduleHomePlayerSafeAreaSync);
  });
  updateHomePlayerSafeArea();
})();
function setHomeControlsLocked(locked) {
  ...
  scheduleHomePlayerSafeAreaSync();
}
...
async function playHomeDaily() {
  ...
  homeSuppressed = false;
  setHomeControlsLocked(false);
  if (!hasAnyConnectedMusicSource() && !homeDiscoverState.loggedIn) {
...
async function playHomePrivateRadio() {
  ...
  if (item && item.id) {
    await loadPlaylistIntoQueueById(playlistPanelProviderId(item.provider, item.id, item.serverId), true, item.name || '私人雷达');
    return;
  }
  openHomeLibrary();
}
...
function openHomePlaylist(index) {
  ...
  loadPlaylistIntoQueueById(playlistPanelProviderId(item.provider, item.id, item.serverId), true, item.name || '');
}
