function loggedProviderCount() {
  return ['navidrome', 'netease', 'qq', 'kugou', 'qishui', 'spotify'].filter(function (key) { return hasPlatformLogin(key); }).length;
}
function updateUserModalUi() {
  ...
  if (vipEl) {
    if (activeAccountProvider === 'netease') {
      ...
      vipEl.textContent = '用户: ' + ((st && st.userId) || '-') + '  /  ' + (st && st.insecure ? 'HTTP 不安全连接' : 'HTTPS 连接');
      vipEl.style.color = st && st.insecure ? 'rgba(255,190,135,0.82)' : 'rgba(92,176,230,0.78)';
    } else {
  ...
  ['netease', 'qq', 'kugou', 'qishui', 'spotify', 'navidrome', 'both'].forEach(function (key) {
  ...
  if (logoutBtn) logoutBtn.textContent = '管理 Navidrome 服务器';
  if (hint) hint.textContent = '管理当前 Navidrome 服务器连接与账号状态。';
}
function showUserModal() {
  if (!(typeof navidromeIsConnected === 'function' && navidromeIsConnected())) return openNavidromeServerModal();
  updateUserModalUi();
...
function setActiveAccountProvider(provider) {
  provider = provider === 'navidrome' ? 'navidrome' : (provider === 'qq' ? 'qq' : (provider === 'kugou' ? 'kugou' : (provider === 'qishui' ? 'qishui' : (provider === 'spotify' ? 'spotify' : 'netease'))));
  if (!hasPlatformLogin(provider)) {
    if (provider === 'navidrome') openNavidromeServerModal();
    else openProviderLogin(provider);
    return;
  }
...
async function doLogout() {
  ...
  activeAccountProvider = firstLoggedProvider();
  userPlaylists = (navidromePlaylists || []).concat(qqPlaylists, kugouPlaylists || [], qishuiPlaylists || [], spotifyPlaylists || []);
  playlistCatalogRevision += 1;
  myPodcastCollections = [];
  myPodcastItems = {};
  likedSongMap = {};
