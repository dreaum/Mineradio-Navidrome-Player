...
function onUserBtnClick(clickEvent) {
...
var ACCOUNT_PROVIDER_KEYS = ['navidrome', 'netease', 'qq', 'kugou', 'qishui', 'spotify'];
...
function normalizeAccountProviderKey(provider) {
  return provider === 'navidrome' ? 'navidrome' : (provider === 'qq' ? 'qq' : (provider === 'kugou' ? 'kugou' : (provider === 'qishui' ? 'qishui' : (provider === 'spotify' ? 'spotify' : 'netease'))));
}
...
function accountProviderExternalRenderList() {
  ...
  return order.filter(function (provider) { return provider !== 'navidrome' && selected.indexOf(provider) >= 0; });
}
...
function platformMeta(provider) {
  ...
  if (provider === 'navidrome') return { key: 'navidrome', short: 'ND', label: 'Navidrome', app: 'Navidrome 服务器', dot: 'navidrome' };
  return { key: 'netease', short: 'NE', label: '网易云音乐', app: '网易云音乐 App', dot: 'netease' };
}
function platformStatus(provider) {
  if (provider === 'navidrome') {
    var server = typeof navidromeCurrentServerProfile === 'function' ? navidromeCurrentServerProfile() : null;
    return {
      provider: 'navidrome',
      loggedIn: typeof navidromeIsConnected === 'function' && navidromeIsConnected(),
      nickname: server && (server.name || server.url) || 'Navidrome',
      userId: server && server.username || '',
      insecure: !!(server && server.insecure)
    };
  }
...
function hasAnyConnectedMusicSource() {
  return hasAnyPlatformLogin() || !!(typeof navidromeIsConnected === 'function' && navidromeIsConnected());
}
function firstLoggedProvider() {
  ...
  return typeof navidromeIsConnected === 'function' && navidromeIsConnected() ? 'navidrome' : 'netease';
}
function providerAvatarSrc(provider, status) {
  status = status || platformStatus(provider) || {};
  if (status.avatar) return avatarSrc(status.avatar);
  var meta = platformMeta(provider);
  var fill = provider === 'qq' ? '#bfd66b' : (provider === 'kugou' ? '#56e0ff' : (provider === 'qishui' ? '#45d68f' : (provider === 'spotify' ? '#1ed760' : (provider === 'navidrome' ? '#5cb0e6' : '#d95b67'))));
  var bg = provider === 'qq' ? '#11150b' : (provider === 'kugou' ? '#071722' : (provider === 'qishui' ? '#071a12' : (provider === 'spotify' ? '#06140a' : (provider === 'navidrome' ? '#07131b' : '#180b0f'))));
...
function bindTopAccountPillSorting() {
  ...
  function finish(e) {
    ...
    if (!dragged && provider && provider !== 'navidrome') loginProvider = provider;
