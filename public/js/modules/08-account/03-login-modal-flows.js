...
var LOGIN_WORKFLOW_PROVIDERS = ['netease', 'qq', 'kugou', 'qishui', 'spotify', 'navidrome'];
...
function normalizeLoginProviderKey(provider) {
  return provider === 'qq' ? 'qq' : (provider === 'kugou' ? 'kugou' : (provider === 'qishui' ? 'qishui' : (provider === 'spotify' ? 'spotify' : (provider === 'navidrome' ? 'navidrome' : 'netease'))));
}
function loginProviderSupportsCookieMode(provider) {
  provider = normalizeLoginProviderKey(provider);
  return provider !== 'spotify' && provider !== 'qishui' && provider !== 'navidrome';
}
function loginProviderOfficialModeText(provider) {
  provider = normalizeLoginProviderKey(provider);
  if (provider === 'navidrome') return { title: '服务器', sub: '管理 Navidrome 服务器连接' };
...
function providerHasLiveLogin(provider) {
  provider = normalizeLoginProviderKey(provider);
  if (provider === 'navidrome') return typeof navidromeIsConnected === 'function' && navidromeIsConnected();
...
function loginWorkflowProviderOrder() {
  var order;
  try { order = accountProviderOrder(); } catch (e) { order = []; }
  LOGIN_WORKFLOW_PROVIDERS.forEach(function (provider) { if (order.indexOf(provider) < 0) order.push(provider); });
  return order;
}
...
function updateLoginProviderUi() {
  ...
  if (isNavidrome) {
    [neteaseBtn, qqBtn, kugouBtn, qishuiBtn, spotifyBtn].forEach(function (button) { if (button) button.classList.remove('active'); });
    if (navidromeBtn) navidromeBtn.classList.add('active');
    if (title) title.textContent = '连接 Navidrome 服务器';
    if (desc) desc.innerHTML = '在同一套 Mineradio 玻璃模态框中添加、验证和切换 Navidrome 服务器。';
    ...
    if (st) {
      ...
      st.textContent = navidromeIsConnected() ? 'Navidrome 已连接，可管理服务器配置' : '点击下方按钮添加 Navidrome 服务器';
    }
    if (refreshBtn) {
      refreshBtn.disabled = false;
      refreshBtn.textContent = '打开服务器连接';
      refreshBtn.onclick = openNavidromeServerModal;
    }
    updateLoginNodeGraphUi();
    return;
  }
...
function toggleQQCookiePanel() {
  if (loginProvider === 'navidrome') return openNavidromeServerModal();
