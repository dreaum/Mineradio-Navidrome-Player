...
function makeContentListManager() {
  ...
  function drawPanel() {
    ...
    var playableCount = allTracks.filter(function (song) { return song && song.id && song.type !== 'podcast-radio' && !isNavidromeCatalogItem(song); }).length;
    ...
    var countLabel = contentKind === 'catalog'
      ? (contentCount ? (contentCount + ' 个项目') : (isLoading ? '正在载入' : '暂无曲库项目'))
      : contentKind === 'podcast'
      ? (contentCount ? (contentCount + ' 项播客内容') : (isLoading ? '正在载入' : '暂无播客内容'))
      : (playableCount ? (playableCount + ' 首歌曲') : (isLoading ? '正在载入' : '暂无可播放歌曲'));
    var loadedCount = contentKind === 'catalog' ? contentCount : playableCount;
    if (contentKind !== 'podcast' && contentTotalCount && contentTotalCount > loadedCount) {
      countLabel = loadedCount + '/' + contentTotalCount + (contentLoadingMore ? ' loading' : ' loaded');
    }
  ...
  function isNavidromeCatalogItem(song) {
    return !!(song && (song.type === 'navidrome-album' || song.type === 'navidrome-artist'));
  }

  function openNavidromeCatalogItem(row) {
    var item = row && row.song;
    if (!isNavidromeCatalogItem(item) || !item.id || !shelfManager) return false;
    ...
    return shelfManager.openExternalContent(
      ...
      String(item.name || item.title || (item.type === 'navidrome-artist' ? '艺术家' : '专辑')),
      {
        ...
        cover: item.cover || item.coverPath || '',
        trackCount: Number(item.songCount || item.albumCount) || 0
      }
    );
  }
  ...
  function contentPageUrl(offset) {
    ...
    if (contentSource.provider === 'navidrome') return 'navidrome';
    if (contentSource.provider === 'navidrome-songs') return 'navidrome-songs';
  ...
  async function loadMoreContentRows(reason) {
    ...
    panelDirty = true;
    try {
      var r = contentSource.provider === 'navidrome'
        ? await navidromePlaylistPage(navidromePlaylistIdentity(contentSource.serverId, contentSource.id), contentNextOffset, PLAYLIST_LAZY_BATCH_SIZE)
        : (contentSource.provider === 'navidrome-songs'
          ? await navidromeSongCollectionPage(contentNextOffset, PLAYLIST_LAZY_BATCH_SIZE)
          : await apiJson(url));
  ...
  function drawRow(row, song, isCenter) {
    ...
    var isCatalogItem = isNavidromeCatalogItem(song);
    var playable = !!(song && song.id && !isPodcastRadio && !isCatalogItem);
    var actionReady = playable || isPodcastRadio || isCatalogItem;
    ctx.clearRect(0, 0, W, H);
    ...
    var textMax = actionReady && isCenter ? ((isPodcastRadio || isCatalogItem) ? btnX - textX - 24 : likeX - textX - 24) : W - textX - 42;
    ...
    if (isCenter && actionReady) {
      if (!isPodcastRadio && !isCatalogItem) {
      ...
      ctx.fillText(isCatalogItem ? '打开' : '播放', btnX + 36, btnY + 29);
    }
    row.texture.needsUpdate = true;
  }
  ...
  return {
    ...
    open: async function (playlistId, title, fromCard) {
      ...
      var navidromeSongCollection = String(playlistId || '') === 'navidrome-songs';
      var navidromePlaylist = !navidromeSongCollection && String(playlistId || '').indexOf('navidrome:') === 0 ? parseNavidromePlaylistIdentity(playlistId) : null;
      var navidromeCatalogKind = navidromePlaylist && typeof navidromeCatalogPlaylistKind === 'function'
        ? navidromeCatalogPlaylistKind(navidromePlaylist.playlistId)
        : '';
      contentKind = podcastCollectionKey ? 'podcast' : (navidromeCatalogKind && navidromeCatalogKind !== 'album' ? 'catalog' : 'playlist');
      contentSource = podcastCollectionKey ? null : {
        provider: navidromeSongCollection ? 'navidrome-songs' : (navidromePlaylist ? 'navidrome' : (qqPlaylistId ? 'qq' : (kugouPlaylistId ? 'kugou' : (qishuiPlaylistId ? 'qishui' : (spotifyPlaylistId ? 'spotify' : 'netease'))))),
        id: navidromePlaylist ? navidromePlaylist.playlistId : (qqPlaylistId || kugouPlaylistId || qishuiPlaylistId || spotifyPlaylistId || playlistId),
        serverId: navidromePlaylist ? navidromePlaylist.serverId : (navidromeSongCollection ? navidromeCurrentServerId() : '')
      };
      // 拉取歌单/播客集合
      var r = null;
      try {
        r = navidromeSongCollection
          ? await navidromeSongCollectionPage(0, PLAYLIST_LAZY_BATCH_SIZE)
          : (navidromePlaylist
            ? await navidromePlaylistPage(playlistId, 0, PLAYLIST_LAZY_BATCH_SIZE)
    ...
    rowActionAtScreen: function (row, sx, sy) {
      ...
      if (isNavidromeCatalogItem(song)) return u >= 0.82 && v > 0.10 && v < 0.90 ? 'open' : null;
    ...
    playRow: function (row) {
      if (isNavidromeCatalogItem(row && row.song)) return openNavidromeCatalogItem(row);
      // 把整个歌单导入队列, 从这首开始播
      ...
      var playIndex = allTracks.slice(0, idx + 1).filter(function (song) { return song && song.id && !isNavidromeCatalogItem(song); }).length - 1;
      var allSongs = allTracks.filter(function (song) { return song && song.id && !isNavidromeCatalogItem(song); }).map(function (song) {
        return cloneSong(song);
      });
      if (!allSongs.length || playIndex < 0) return;
      var queuePlaylistId = contentSource && contentSource.provider === 'navidrome'
        ? navidromePlaylistIdentity(contentSource.serverId, contentSource.id)
        : (contentSource && contentSource.provider === 'navidrome-songs'
          ? ('navidrome-songs:' + String(contentSource.serverId || navidromeCurrentServerId()))
    ...
    isCatalogRow: function (row) { return isNavidromeCatalogItem(row && row.song); },
    openCatalogRow: function (row) { return openNavidromeCatalogItem(row); }
  };
