function makeShelfManager() {
  ...
  function currentItems() {
    if (hasAnyConnectedMusicSource() && (userPlaylists.length || myPodcastCollections.length)) {
      var source = activePlaylists();
      var items = source.map(function (pl) {
        var provider = normalizePlaylistProvider(pl.provider);
        var sourceLabel = playlistProviderLabel(provider);
        if (provider === 'spotify' && String(pl.id || '').indexOf('spotify:') !== 0) pl = Object.assign({}, pl, { id: 'spotify:' + pl.id });
        var sub = pl.libraryAllSongs
          ? sourceLabel + ' · ' + (pl.trackCount ? String(pl.trackCount) + (pl.trackCountPending ? '+' : '') + ' 首 · 按需加载' : '服务器曲库 · 按需加载')
          : sourceLabel + ' · ' + (pl.trackCount || 0) + ' 首 · 播放 ' + compactCount(pl.playCount || 0);
        return {
          type: 'playlist', title: pl.name, sub: sub,
          cover: pl.cover || '', tag: pl.libraryAllSongs ? '服务器曲库' : ((pl.shelfPane || pl.shelf_pane) === 'fav' || (!(pl.shelfPane || pl.shelf_pane) && pl.subscribed) ? '收藏歌单' : (provider === 'navidrome' ? '服务器歌单' : (provider === 'qishui' ? '汽水歌单' : '我的歌单'))), playlistId: playlistPanelProviderId(provider, pl.id, pl.serverId), provider: provider
        };
      });
...
void main(){ vec4 t = texture2D(uDotTex, gl_PointCoord); if (t.a < 0.02) discard; gl_FragColor = vec4(vC, t.a * vA); }`,
  ...
  function step(direction) {
    ...
    if (!shelfMergesCollections()) {
      if (hasAnyConnectedMusicSource() && userPlaylists.length && shelfPane === 'mine' && atEnd && panes.fav.length) {
        collectionReveal += Math.min(1.5, Math.abs(direction));
        if (collectionReveal >= 3) switchPane('fav');
        return;
      }
      if (hasAnyConnectedMusicSource() && userPlaylists.length && shelfPane === 'fav' && atStart && panes.mine.length) {
        collectionReveal += Math.min(1.5, Math.abs(direction));
  ...
  return {
    ...
    openExternalContent: function (playlistId, title, meta) {
      if (!contentList) contentList = makeContentListManager();
      openCardIdx = -1;
      var virtualCard = { item: Object.assign({ title: title || '', trackCount: 0, cover: '' }, meta || {}) };
      contentList.open(playlistId, title || '歌曲', virtualCard);
      setShelfPinnedOpen(true, true);
      if (typeof updateEmptyHomeVisibility === 'function') updateEmptyHomeVisibility({ forceLoad: false });
      if (typeof setFocusZone === 'function') setFocusZone('shelf-detail', true);
      return true;
