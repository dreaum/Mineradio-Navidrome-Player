...
function toggleFx(key) {
  ...
  if (key === 'audioFocusPause') {
    if (typeof configureAudioFocusPauseMonitor === 'function') configureAudioFocusPauseMonitor();
    if (!fx.audioFocusPause && typeof audioFocusPauseByExternalMedia !== 'undefined') audioFocusPauseByExternalMedia = false;
  }
  ...
  if (key === 'audioFocusPause') showToast(fx.audioFocusPause ? '前台占用自动暂停已开启' : '前台占用自动暂停已关闭');
