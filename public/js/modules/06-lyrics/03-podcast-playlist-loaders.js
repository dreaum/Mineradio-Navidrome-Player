...
function playlistQueueSource(id) {
  var raw = String(id || '');
  if (raw.indexOf('navidrome-songs:') === 0) return { provider: 'navidrome-songs', serverId: raw.slice(17), id: 'navidrome-songs', requestId: raw };
  if (raw.indexOf('navidrome:') === 0) {
    var navidromeIdentity = parseNavidromePlaylistIdentity(raw);
    return { provider: 'navidrome', serverId: navidromeIdentity.serverId, id: navidromeIdentity.playlistId, requestId: raw };
  }
...
async function hydratePlaylistQueueNextPage(reason) {
  ...
  var source = { provider: state.provider, serverId: state.serverId || '', id: state.sourceId, requestId: state.playlistId };
  ...
  var pageRequest = source.provider === 'navidrome'
    ? navidromePlaylistPage(navidromePlaylistIdentity(source.serverId, source.id), offset, limit)
    : (source.provider === 'navidrome-songs'
      ? navidromeSongCollectionPage(offset, limit)
...
async function loadPlaylistIntoQueueById(id, autoplay, title, opts) {
  ...
  try {
    if (!seedTracks.length) {
      r = source.provider === 'navidrome'
        ? await navidromePlaylistPage(navidromePlaylistIdentity(source.serverId, source.id), 0, playlistQueuePageSize(source.provider, true))
        : (source.provider === 'navidrome-songs'
          ? await navidromeSongCollectionPage(0, playlistQueuePageSize(source.provider, true))
  ...
  try {
    ...
    var catalogPlaylist = userPlaylists.find(function (pl) {
      return normalizePlaylistProvider(pl && pl.provider) === source.provider && String(pl && pl.id || '') === String(source.id || '') && (source.provider !== 'navidrome' || String(pl && pl.serverId || '') === String(source.serverId || ''));
    });
