...
function persistentLyricCacheKey(song) {
  ...
  return ['lyrics-v1', provider, song.serverId || '', id, song.name || song.title || '', artist].join('|');
}
...
async function runQueueLyricPrefetch(fromIndex, token) {
  ...
  try {
    ...
    var response = songProviderKey(candidate.song) === 'navidrome'
      ? await navidromeFetchLyricResponse(candidate.song)
      : await apiJson(lyricEndpointForSong(candidate.song));
    if (token !== lyricQueuePrefetchToken) return false;
...
function refreshPersistentLyricCache(song) {
  var request = songProviderKey(song) === 'navidrome'
    ? navidromeFetchLyricResponse(song)
...
async function fetchLyric(songOrId, token, attempt) {
  ...
  try {
    ...
    if (song && songProviderKey(song) === 'navidrome') {
      var navidromeResponse = await navidromeFetchLyricResponse(song);
      var navidromeState = applyFetchedLyricResponse(song, token, navidromeResponse);
      if (!navidromeState || !navidromeState.usableLyric) {
        cancelPendingTrackFallbackLyrics();
