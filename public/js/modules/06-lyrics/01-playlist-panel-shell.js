...
function openPlaylistPanelTab(tab, preserve) {
  ...
  if (tab === 'playlists' && typeof schedulePlaylistPanelVirtualRender === 'function') {
...
function playlistCatalogProviderArray(provider) {
  if (provider === 'navidrome') return navidromePlaylists;
...
function setPlaylistCatalogProviderArray(provider, rows) {
  ...
  if (provider === 'navidrome') navidromePlaylists = rows;
  else if (provider === 'netease') neteasePlaylists = rows;
...
function playlistCatalogProviderLoggedIn(provider) {
  if (provider === 'navidrome') return navidromeIsConnected();
...
function syncPlaylistPanelSourceLabel() {
  var chip = document.querySelector('#pl-pane .queue-toolbar .queue-chip');
  if (!chip) return;
  var navConnected = typeof navidromeIsConnected === 'function' && navidromeIsConnected();
  var navConfigured = typeof navidromeHasConfiguredServer === 'function' && navidromeHasConfiguredServer();
  var legacyConnected = typeof hasAnyPlatformLogin === 'function' && hasAnyPlatformLogin();
  if (navConnected && legacyConnected) chip.textContent = '已连接音源的歌单 · 含 Navidrome';
  else if (navConnected) chip.textContent = 'Navidrome 服务器歌单 · 全部歌曲';
  else if (navConfigured) chip.textContent = 'Navidrome 尚未连通 · 请重新输入密码';
  else chip.textContent = '登录后显示已连接音源的歌单';
}
...
function mergePlaylistCatalogRows(existing, incoming, provider) {
  ...
  (existing || []).concat(incoming || []).forEach(function (pl) {
    ...
    pl.source = provider;
    var key = provider + ':' + String(pl.serverId || '') + ':' + String(pl.id || '');
...
function rebuildUserPlaylistsFromCatalog(opts) {
  opts = opts || {};
  userPlaylists = navidromePlaylists.concat(neteasePlaylists, qqPlaylists, kugouPlaylists, qishuiPlaylists, spotifyPlaylists);
...
async function loadPlaylistCatalogProviderPage(provider, reason) {
  ...
  try {
    var r = provider === 'navidrome'
      ? { playlists: await navidromeLoadPlaylists(!!first), total: 0, nextOffset: 0, hasMore: false }
      : await apiJson(url, { timeoutMs: 15000 });
...
function requestNextPlaylistCatalogPage(reason) {
  ...
  var order = ['navidrome', 'netease', 'spotify', 'qq', 'kugou', 'qishui'];
...
async function refreshUserPlaylists(force) {
  if (!loginStatus.loggedIn && !qqLoginStatus.loggedIn && !kugouLoginStatus.loggedIn && !qishuiLoginStatus.loggedIn && !spotifyLoginStatus.loggedIn && !navidromeIsConnected()) {
    resetPlaylistPanelRenderLimit();
    var navConfigured = typeof navidromeHasConfiguredServer === 'function' && navidromeHasConfiguredServer();
    document.getElementById('pl-list').innerHTML = navConfigured
      ? '<div class="playlist-panel-notice navidrome-disconnected">Navidrome 已配置，但当前未连通。<br><button type="button" class="fx-mini-btn ghost" onclick="openNavidromeServerModal()">重新输入密码并连接</button></div>'
  ...
  var catalogNeedsNewProvider = playlistCatalogSyncState.loading && ['netease', 'qq', 'kugou', 'qishui', 'spotify', 'navidrome'].some(function (provider) {
  ...
  ['netease', 'qq', 'kugou', 'qishui', 'spotify', 'navidrome'].forEach(function (provider) {
