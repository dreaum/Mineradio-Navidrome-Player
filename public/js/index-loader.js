...
(function loadMineradioIndexModules() {
  ...
  window.__mineradioModulesReady = false;
  window.__mineradioModulesError = '';
  window.__mineradioModuleErrorListener = function (event) {
    window.__mineradioModulesError = [event && event.message, event && event.filename, event && event.lineno, event && event.colno]
      .filter(Boolean).join(' @ ');
  };
  window.addEventListener('error', window.__mineradioModuleErrorListener);
  const modulePaths = [
    ...
    'js/modules/05-playback/14a-audio-focus-pause.js',
    ...
    'js/modules/08-account/06-navidrome-provider.js',
  ...
  script.text = modulePaths.map(readModule).join('\n') + '\nwindow.__mineradioModulesReady = true; window.removeEventListener(\'error\', window.__mineradioModuleErrorListener);\n//# sourceURL=mineradio-index-modules.js\n';
